...
var f = module.exports = {
	...
	*/
	genMd5:function(target,source,encoding){
		...
		var cssregExp = eval("/^"+jdf.config.outputDirName+"\\/css/").test(target);
		var jsregExp = eval("/^"+jdf.config.outputDirName+"\\/js/").test(target);
		var suf = (/\.css$/.test(target) || /\.js$/.test(target));
		var isMd5 = (jdf.config['output'].cssMd5 || jdf.config['output'].jsMd5);
		var isWidget = eval("/^"+jdf.config.outputDirName+"\\/widget/").test(target);

		if( isMd5 && suf && (cssregExp||jsregExp) && !isWidget){
			var originCssPath = target.split(jdf.config.outputDirName+"/css/")[1];
			var originJsPath = target.split(jdf.config.outputDirName+"/js/")[1];
			...
			if(suffix=="css" && jdf.config['output'].cssMd5){
				...
				var md5CssPath = target.split(jdf.config.outputDirName+"/css/")[1];
				var revcss = process.cwd()+"/"+jdf.config.outputDirName+'/revcss.json';
			...
			if(suffix=="js" && jdf.config['output'].jsMd5){
				target = target.substring(0,target.lastIndexOf('/')+1);
				target = target+fileName+md5Str+"."+suffix;
				f.write(target,fs.readFileSync(source), encoding);

				var md5JsPath = target.split(jdf.config.outputDirName+"/js/")[1];
				var revjs = process.cwd()+"/"+jdf.config.outputDirName+'/revjs.json';
				if(!f.exists(revjs)){
					var jsCon = {};
	...
	*/
	md5Replace:function(sourceCode){
		...
		if(jdf.config['output'].cssMd5){
			if(f.exists(process.cwd()+"/build/revcss.json")){
				var cssRev = fs.readFileSync(process.cwd()+"/build/revcss.json");
		...
		if(jdf.config['output'].jsMd5){
			if(f.exists(process.cwd()+"/build/revjs.json")){
				var jsRev = fs.readFileSync(process.cwd()+"/build/revjs.json");
		...
		if(source && f.filter(source, null, exclude)){
			...
			if(f.isDir(source)){
				...
				if(uncover && f.exists(target)){
					...
					if( (/\.html$/.test(target)||/\.ftl$/.test(target) ) && target.indexOf('.jdf-temp')==-1){
		    			inline(target, {
							compress: true,
							...
							var jdf = require('./jdf.js');
							var compress = require('./compress.js');
							var sourceCode = compress.cssImagesUrlReplace(target, html, jdf.config.cdn);
							f.write(target, sourceCode, encoding);
							sourceCode = compress.addJsDepends(target,sourceCode);
							if(jdf.config['output'].cssMd5 || jdf.config['output'].jsMd5){
